...
#run in terminal: python dat_to_py.py bif_line.dat
    ...
    for j in range(len(y)) :
        ...
        for i in range(len(x)) :
            if x[i] > g_min :
                ...
                period = (peaks[-1] - peaks[-2])*STEP
                # Frequency
                # We consider that time unit here is in the order of 10^-2 seconds,
                # (although one could consider it as being 10^-1 seconds depending 
                # on the unit used for tau_m), and re-adjust to the order of seconds
                Fq[j,i] = (1/period)*100
    with open('bif_overlay.txt', 'w') as f:
        csv.writer(f, delimiter=' ').writerows(Fq)


Fq[np.where(Fq < 1)] = np.nan
...
plt.pcolormesh(X, Y, Fq, shading='auto', vmin=6, vmax=33)
cm = plt.colorbar(label='spiking frequency (a.u.)')
