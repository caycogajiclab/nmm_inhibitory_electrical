...
for filename in args.files :
    ...
    with open(filename, newline='') as file:
        ...
        for row in datareader:
            ...
            if (last_stability != int(row[3]) and len(g[-1]) > 1) :
                g.append([])
                J.append([])
                if last_stability != 0 :
                    stability.append(last_stability)
    ...
    for k in range(len(g)) :
        if stability[k] == 1 :
            plt.plot(g[k], J[k], color=colors[col], linestyle=s[0], label=f'$\Delta={delta[col]}$')
        if stability[k] == 2 :
            plt.plot(g[k], J[k], color=colors[col], linestyle=s[1], label=f'$\Delta={delta[col]}$')
        if stability[k] == 3 :
            plt.plot(g[k], J[k], color=colors[col], linestyle=s[2], label=f'$\Delta={delta[col]}$')
    col += 1
...
#plt.title('Increased heterogeneity delays oscillations', size=18, pad=20)

plt.tight_layout()
plt.savefig('bif_delta.png', dpi=600)
plt.show()
